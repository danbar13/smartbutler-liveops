...
with tab1:
    ...
    if all_reports:
        with st.expander(t("tab1_arch_expander", lang, count=len(all_reports)), expanded=False):
            ...
            for r in all_reports:
                ...
                if is_active:
                    ...
                    sel_icon = f'<a href="?report_id={r["id"]}&lang={lang}" target="_self" title="{t("t1_col_select", lang)}" style="text-decoration:none;"><span style="display:inline-flex; align-items:center; justify-content:center; width:20px; height:20px; border-radius:50%; border:2px solid #64748b; background:rgba(255,255,255,0.04); color:#94a3b8; font-size:11px; cursor:pointer;">○</span></a>'

                status_circle = '<span style="font-size:15px;">🟢</span>' if is_active else ''

                html.append(f"""<tr onclick="window.location.search='?report_id={r['id']}&lang={lang}'" style="cursor: pointer; {row_bg}">
